...
async function listUsers(request, response) {
    ...
    connection.query('select * from usuarios where tipo_usuario = "Admin";', (err, results) => { 
...
async function storeUser(request, response) {
    // Preparar o comando de execução no banco
    const query = 'INSERT INTO usuarios(email, senha, tipo_usuario, nome_empresa,telefone,endereco,instagram,foto) VALUES(?,?, ?,?,?,?,?,?);'; 
...
async function updateUserEm(request, response) {
    // Preparar o comando de execução no banco
    const query = "UPDATE usuarios SET `email` = ?, `senha`=?, `nome_empresa`=?, `telefone`=?, `endereco`=?, "+
    "`instagram`=?, `foto` = ? WHERE `id` = ?";
